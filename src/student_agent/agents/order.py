...
class OrderAgent(Agent):
    ...
    async def handle(self, ctx: CaseContext, message: A2AMessage) -> A2AMessage:
        ...
        # Seller records are a fallback when item rows lack seller IDs: citing them for seller
        # responsibility adds a 7th call (over the efficiency budget) for almost no evidence gain.
        if rows and not payload["seller_ids"]:
        ...
        # Product context only supports an unavailable-product conclusion; skip it otherwise.
        ...
        ) and (episode_status == "unavailable" or claimed_topic(ctx.case) == PRODUCT_TOPIC)
        if wants_products:
