...
class PaymentAgent(Agent):
    ...
    async def handle(self, ctx: CaseContext, message: A2AMessage) -> A2AMessage:
        ...
        # Refund lifecycle is only fetched when the complaint is about a refund; for other
        # orders the tool has no data or only refunds of unrelated episodes.
        refund = None
        if claimed_topic(ctx.case) in REFUND_TOPICS:
