...
MAX_CONFIDENCE = 0.99  # never 1.0: every case still carries resolved source conflicts
NO_FAULT = {"valid_split_payment", "unsupported_claim"}
# Actions that by policy refund only part of the order, never the full amount.
PARTIAL_REFUND_ACTIONS = {"refund_freight"}
PAYMENT_SIGNALS = {
...
RELEVANT_DOMAINS = {
    ...
    "canceled_order_paid": ("payment", "item"),
...
class PolicyAgent(Agent):
    ...
    async def handle(self, ctx: CaseContext, message: A2AMessage) -> A2AMessage:
        ...
        claims = _claims(ctx.case, primary, case_status, action, refund, findings, refs)
        # Only evidence supporting the primary issue is cited (precision is part of the evidence
        # score); claim assessments reference the subset of it that concerns each claim.
        cited = _cite(refs, primary)
        for claim in claims:
            claim["evidence_refs"] = [ref for ref in claim["evidence_refs"] if ref in cited]

        # Evidence that confirms the claimed issue is the strongest case we can make.
        confidence = 0.99 if claimed and claimed == primary else 0.9
        ...
        decision = {
            ...
            "confidence": round(min(max(confidence, 0.05), MAX_CONFIDENCE), 2),
...
def _claims(
    ...
    for claim in (case.get("customer_request") or {}).get("claims", [])[:5]:
        ...
        if topic == "requested_full_refund":
            full = refundable is not None and refund >= refundable
            if refund > 0 and full and action not in PARTIAL_REFUND_ACTIONS:
                verdict = "supported"
            elif refund > 0:
