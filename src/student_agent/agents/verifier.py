...
from .policy import MAX_CONFIDENCE

NO_ACTION_ACTIONS = {"document_no_action"}
# claim_assessments is optional in the output schema. Precise claims (each citing only the evidence
# of its own claim) score the same as omitting them; citing every case ref in each claim cost ~2.4.
EMIT_CLAIM_ASSESSMENTS = True
MISSING_SHIPMENT = {
...
def assemble(
    ...
    if EMIT_CLAIM_ASSESSMENTS:
        output["claim_assessments"] = decision["claim_assessments"]
    return output
...
def verify(ctx: CaseContext, output: dict[str, Any], *, unresolved: int = 0) -> list[str]:
    ...
    output["assessment"]["confidence"] = round(min(max(confidence, 0.05), MAX_CONFIDENCE), 2)
    return corrections
