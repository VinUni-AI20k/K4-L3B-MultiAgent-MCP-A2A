...
class ShipmentAgent(Agent):
    ...
    async def handle(self, ctx: CaseContext, message: A2AMessage) -> A2AMessage:
        ...
        summary = None
        if not episode or delivered_late(episode) or claimed_topic(ctx.case) in SUMMARY_TOPICS:
...
def _verdict(
    ...
    if status in {"canceled", "unavailable"}:
        # Never handed over for delivery, so no delay can be attributed to seller or carrier.
        return "on_time", []
