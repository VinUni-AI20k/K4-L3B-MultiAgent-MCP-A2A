...
4. **Policy & Conflict Resolution**: Specialist findings are collected and forwarded to `PolicyAgent`. Powered by Gemini Flash Lite (`gemini-3.5-flash-lite` / `gemini-flash-lite-latest`), the Policy Agent cross-references evidence against `EC_POLICY_V2` rules and resolves buyer-seller-carrier conflicts.
...
| **Order / Item Agent** | `order_id`, scope | Validate items, prices, seller IDs, catalog categories | `get_order`, `get_order_items`, `get_sellers`, `get_product_context` | `OrderFindings` -> Policy Agent |
...
| :--- | :---: | :--- | :--- |
| **MCP Tool Timeout** | 2 retries | Exponential backoff (1s, 2s) | Log warning attribute |
